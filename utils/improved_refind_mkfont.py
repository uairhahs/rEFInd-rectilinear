...
def create_refind_font(font_path, cell_height, output_filename):
    ...
    for i in range(CHAR_START, CHAR_END + 1):
        ...
        try:
            ...
            draw.text(
                (cell_x + x_offset, y_offset), char, font=font, fill=(0, 0, 0, 255)
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a rEFInd-compatible font PNG."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=28,
        help="The desired character cell height in pixels (default: 28).",
    )
    parser.add_argument(
        "output_file", help="The output PNG filename (e.g., spacemono-28.png)."
    )
